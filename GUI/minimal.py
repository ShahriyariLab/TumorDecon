...
def browse_for_file():
    ...
    global rna_df
    rna_df = td.read_pancancer_rna_file(name)
    # Remove nans:
...
if __name__ == '__main__':
    ...
    # Upload RNA Data:
    Label(root,text="1. Upload RNA Data:", font="Times 16 bold").grid(row=1,column=0, sticky = W)
    cpane = CollapsiblePane(root, 'Collapse Instructions', 'Instructions for uploading your own RNA data file:')
    cpane.grid(row=2,sticky = W)
    Label(cpane.frame,text="- Data file extension must be either .csv, .xlsx, or .txt").grid(row=3,sticky = W)
    Label(cpane.frame,text="- First column must contain a list of genes, referenced by their Hugo Symbol").grid(row=4,sticky = W)
    Label(cpane.frame,text="- Each remaining column should be the rna expression values of each gene for a patient").grid(row=5,sticky = W)
    Label(cpane.frame,text="- First row of file should be a header, containing column names (patient identifiers)").grid(row=6,sticky = W)
    Button(root, text='Browse...', command=browse_for_file).grid(row=7,column=0)
    rna_file_selection = tk.Label(root, text=" ")
    rna_file_selection.grid(row=8, column=0)

    # Pad:
    Label(root, text = "        ").grid(row=9,column=0)

    # Select Method:
    Label(root, text="2. Choose tumor deconvolution method:", font="Times 16 bold").grid(row=10, column=0, sticky = W)
    m = StringVar()
    ...
    OptionMenu(root, m, *methods, command=show_method_options).grid(row=11, sticky = W)
    ...
    Label(root, text="3. Additional Options:", font="Times 16 bold").grid(row=16,column=0, sticky=W)
    s, u, d = StringVar(), StringVar(), StringVar()
    s.set('LM22')
    u.set('ssGSEA paper gene set')
    d.set('None')
    sig_label = Label(root, text="Signature Matrix:")
    sig_label.grid(row=17, column=0, sticky=W)
    # sig_label.grid_remove()
    sig_mat_optionslist = ["LM22", "Upload your own..."]
    sig_dropdown = OptionMenu(root, s, *sig_mat_optionslist, command=show_sig_options)
    sig_dropdown.grid(row=18, sticky = W)
    # sig_dropdown.grid_remove()
    cpane2 = CollapsiblePane(root, 'Collapse Instructions', 'Instructions for uploading your own signature matrix data file:')
    cpane2.grid(row=19,sticky = W)
    cpane2.grid_remove()
    Label(cpane2.frame,text="- Data file extension must be either .csv, .xlsx, or .txt").grid(row=20,sticky = W)
    Label(cpane2.frame,text="- First column must contain a list of genes, referenced by their Hugo Symbol").grid(row=21,sticky = W)
    Label(cpane2.frame,text="- Each remaining column should be the signature expression value in the column's cell type, for each gene").grid(row=22,sticky = W)
    Label(cpane2.frame,text="- First row of file should be a header, containing column names").grid(row=23,sticky = W)
    sig_browse = Button(root, text="Browse...", command=get_custom_signature)
    sig_browse.grid(row=24, column=1)
    sig_browse.grid_remove()
    sig_selection = Label(root, text = "")
    sig_selection.grid(row=25,column=0)
    # sig_selection.grid_remove()
    ...
    upgene_dropdown.grid(row=18, sticky = W)
    upgene_dropdown.grid_remove()
    cpane3 = CollapsiblePane(root, 'Collapse Instructions', 'Instructions for uploading your own gene sets data file:')
    cpane3.grid(row=19,sticky = W)
    cpane3.grid_remove()
    Label(cpane3.frame,text="- Data file extension must be either .csv, .xlsx, or .txt").grid(row=20,sticky = W)
    Label(cpane3.frame,text="- First row should be a list of cell types (column names)").grid(row=21,sticky = W)
    # File should have columns named for each cell type,
    Label(cpane3.frame,text="- Rows should contain Hugo Symbols for genes to be considered as up-regulated for the given cell type").grid(row=22,sticky = W)
    Label(cpane3.frame,text="- If not all cell types have the same number of up-regulated genes, excess rows in that column should be coded as 'NaN'").grid(row=23,sticky = W)
    upgenes_browse = Button(root, text="Browse...", command=get_custom_up_geneset)
    upgenes_browse.grid(row=24, column=1)
    upgenes_browse.grid_remove()
    upgenes_selection = Label(root, text = "")
    upgenes_selection.grid(row=25,column=0)
    upgenes_selection.grid_remove()

    downgene_label = Label(root, text="Down-Regulated Gene Set (optional, for bidirectional SingScore):")
    downgene_label.grid(row=26, column=0, sticky=W)
    downgene_label.grid_remove()
    ...
    downgene_dropdown.grid(row=27, sticky = W)
    downgene_dropdown.grid_remove()
    downgenes_browse = Button(root, text="Browse...", command=get_custom_down_geneset)
    downgenes_browse.grid(row=28, column=1)
    downgenes_browse.grid_remove()
    downgenes_selection = Label(root, text = "")
    downgenes_selection.grid(row=29,column=0)
    downgenes_selection.grid_remove()

    # Alpha (occupies same space as normalization & downgenes)
    alpha_label = Label(root, text="Alpha value:")
    alpha_label.grid(row=26, column=0, sticky=W)
    alpha_label.grid_remove()
    alpha = Entry(root)
    alpha.grid(row=27, column=0, sticky=W)
    alpha.insert(0, "1.0")
    alpha.grid_remove()

    # Pad:
    Label(root, text="      ").grid(row=28, column=0)

    # Save results:
    Label(root, text="4. Choose location to save results:", font="Times 16 bold").grid(row=30,column=0,sticky = W)
    Button(root, text="Choose folder...", command=get_save_location).grid(row=31,column=0)
    print_save_loc = tk.Label(root, text=" ")
    print_save_loc.grid(row=32, column=0)

    # Pad:
    Label(root, text="    ").grid(row=33, column=0)

    Label(root, text="5. Run TumorDecon:", font="Times 16 bold").grid(row=34,column=0,sticky=W)
    Button(root, text="Run!", command=run_tumor_decon).grid(row=35,column=0)

    status = tk.Label(root, text=" ")
    status.grid(row=36, column=0)
